...
reset-mb-h2:
	...
	@docker run --rm \
		...
		-c "printf 'ALTER TABLE REPORT_CARD ALTER COLUMN ID RESTART WITH 9000000;\nALTER TABLE REPORT_DASHBOARD ALTER COLUMN ID RESTART WITH 9000000;\nALTER TABLE REPORT_DASHBOARDCARD ALTER COLUMN ID RESTART WITH 9000000;\n' \
		    > /tmp/fix_seq.sql && \
		    java -cp /app/metabase.jar org.h2.tools.RunScript \
		    -url 'jdbc:h2:/metabase-data/metabase.db/metabase.db' \
		    -user '' -password '' \
		    -script /tmp/fix_seq.sql && \
		    echo 'Sequences reset to 9000000 — OK'"
