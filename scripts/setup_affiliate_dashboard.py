...
def template_tags(affiliate_field_id):
    """
    affiliate_name – field filter (dimension); Metabase generates equality SQL.
    from_date / to_date – simple date variables; value is passed as-is to SQL.
      The DB stores dates exactly as YYYY-MM-DD (from filenames), so a direct
      string comparison against the report_date DATE column works with no
...
# from_date / to_date are plain date variables.  Metabase passes the value the
# user picks as a YYYY-MM-DD string, which MySQL compares directly to the DATE
# column.  No timezone math needed: the DB dates come from filenames.
WHERE = """
    WHERE 1=1
    [[AND {{affiliate_name}}]]
